...
async function createMetadataTemplates() {
  ...
  try {
    await client.metadata.createTemplate('projectMetadata', [
      {
        type: 'string',
        key: 'projectCode',
    ...
    if (error.response?.body?.code === 'conflict') {
      ...
      console.error('❌ Failed to create projectMetadata template:', error.message, '\n');
    }
  }
  ...
  try {
    await client.metadata.createTemplate('experimentMetadata', [
      {
        type: 'string',
        key: 'experimentId',
    ...
    if (error.response?.body?.code === 'conflict') {
      ...
      console.error('❌ Failed to create experimentMetadata template:', error.message, '\n');
    }
  }
  ...
  try {
    await client.metadata.createTemplate('entryMetadata', [
      {
        type: 'string',
        key: 'entryId',
    ...
    if (error.response?.body?.code === 'conflict') {
      ...
      console.error('❌ Failed to create entryMetadata template:', error.message, '\n');
    }
  }
